...
layui.config({
        ...
        ,table = {
            config: {//全局配置项,表格级别
                ...
                ,cols:{//节点级别的附加字段
                    ...
                    ,className:['first','sec','third','fourth','five']
    ...
    Class.prototype.config = {
        ...
        ,onClickTreeHead:null //展开收起单击事件
        ,onClickNextPage:null //展开显示更多单击事件
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var isTreeNode=(o.children&&o.children.length>0) || (o.inviteRecord&&o.inviteRecord.totalNum > 0); //有改动
        var temhtml='<div style="float: left;height: 28px;line-height: 28px;padding-left: '+
            ...
            +function () {//图标或占位符
                ...
                var temTreeIsOpen=o[table.config.cols.isOpen]?"static/dist/img/up.png":"static/dist/img/down.png";
                if(isTreeNode){//父节点
                    temTreeHtml='<i class="layui-icon layui-tree-head"><img src="'+temTreeIsOpen+'" class="mr5"></i>'
    ...
    Class.prototype.treeNodeOpen=function (o,isOpen) {
        ...
        if(o[table.config.cols.isOpen]){//打开状态
            // dbClickI.html('&#xe625;');
            dbClickI.html('<img src="static/dist/img/up.png" class="mr5">');
        }else{
            // dbClickI.html('&#xe623;');
            dbClickI.html('<img src="static/dist/img/down.png" class="mr5">');
        }
    };
    ...
    Class.prototype.treeIconRender=function (o,isHtml) {
        var that = this
            ...
            // ,isTreeNode=(o.children&&o.children.length>0); 改动
            ,isTreeNode=(o.inviteRecord&&o.inviteRecord.totalNum>0);
        var temTreeHtml='';
        if(iconOpen){
    ...
    //源代码
            ...
            //         temTreeHtml=temf();
            //     }else if(o.showMore){ //有改动
            //         temTreeHtml+='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-end" '+iconOpen+'>'+that.config.end+'</i>';
            //     }else{
    ...
    //改动：1-5层级数字
            if(o.lay_level && o.lay_level == 1){
                temTreeHtml+='<i class="layui-tree-'+o[options.idField]+' '+table.config.cols.className[o.lay_level-1]+'"  '+iconOpen+'><img src="static/dist/img/king.png" class="mr5"></i>';
            }else{
                temTreeHtml+='<i class="layui-icon level-circle layui-tree-'+o[options.idField]+' '+table.config.cols.className[o.lay_level-2]+'"  '+iconOpen+'>'+(o.lay_level-1)+'</i>';
            }
    //结束       
    ...
    //新增的加载更多的方法
    Class.prototype.showAddMoreRender = function(o){
         var that = this
            ,options = that.config
            ,iconOpen=options.iconOpen;
        var addHtml = ''
        if(o.showMore){
            addHtml+='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-end" '+iconOpen+'><img src="static/dist/img/add.png"></i>';
        };
        return addHtml;
    }
    ...
    Class.prototype.renderTd=function (param) {
        ...
        if(options.isTree){
            if(options.treeShowName==cols.field){
                ...
                addImgHtml = that.showAddMoreRender(obj);
            }
        ...
        var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            }() +'">'+treeImgHtml+'<p style="width: auto;height: 100%; display:inline-block;margin-right:5px;">'+ function(){
    ...
    Class.prototype.eventsinit = function(){
        ...
        that.elem.on('click','i.layui-tree-head', function(){
            ...
            that.resize();
            typeof options.onClickTreeHead === 'function' && options.onClickTreeHead(index,o);
        });
        //叶节点点击事件（即分页功能，查看）
        that.elem.on('click','i.layui-tree-end', function(){
            var othis = $(this)
                ,index = othis.parents('tr').eq(0).data('index')
                ,options=that.config
                ,datas=table.getDataList(that.key);//数据
            ...
            othis.html('');      //改动：新增 
            // that.treeNodeOpen(o,!o[table.config.cols.isOpen]); // 暂时关闭
            that.resize();
            typeof options.onClickNextPage === 'function' && options.onClickNextPage(index,o);
        });
